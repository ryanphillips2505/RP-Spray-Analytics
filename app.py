...
if process_clicked:
    ...
    try:
        ...
        # Track the current batter using GameChanger's "<Name> at bat" lines
        current_batter = None
        AT_BAT_REGEX = re.compile(r"^(.+?)\s+at\s+bat\b", re.IGNORECASE)
        ...
        for line in lines:
            ...
            # Update current batter context (many GC events like "Bunt" don't repeat the name)
            m_at = AT_BAT_REGEX.match(clean_line)
            if m_at:
                name_chunk = m_at.group(1).strip()
                b_ctx = get_batter_name(name_chunk, current_roster) or get_batter_name(clean_line, current_roster)
                if b_ctx:
                    current_batter = b_ctx
                continue
            ...
            # Only count balls in play after we have a batter (explicit or inferred from context)
            if not is_ball_in_play(line_lower):
                continue

            batter = get_batter_name(clean_line, current_roster) or current_batter
            if batter is None:
                continue
            # Keep context fresh if the name IS on this line
