...
with pd.ExcelWriter(out, engine="openpyxl") as writer:
    ...
    # Coach Notes column font size 12 (if a Coach Notes column exists)
    _coach_headers = {"COACH NOTE", "COACH NOTES", "COACHES NOTE", "COACHES NOTES"}
    for _c in range(1, ws.max_column + 1):
        _h = str(ws.cell(row=1, column=_c).value or "").strip().upper()
        if _h in _coach_headers:
            for _r in range(1, ws.max_row + 1):
                _cell = ws.cell(row=_r, column=_c)
                _cell.font = Font(size=12, bold=(_r == 1))
    ...
    if end_row >= start_row and end_col >= start_col:
        ...
        # heatmap (all numeric stats except GB% / FB% columns if present)
        headers_row = 1
        headers = [str(ws.cell(row=headers_row, column=c).value or "").strip() for c in range(1, end_col + 1)]
        ...
        for c in range(start_col, end_col + 1):
            ...
            ws.conditional_formatting.add(rng, heat_rule)
